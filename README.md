...
| Neural-Pro | WebGPU RAVU-Lite + WebGL2 fallback/Zoom | LGPL-3.0-or-later | WebGPU runs the first RAVU-Lite-AR r3 compute path on supported browsers; WebGL2 remains the visually verified fallback and lazy-loads the much larger RAVU-Zoom-AR r3 hook only when Zoom is selected. |
...
Crisp, Sharpen, and Anime currently prefer the WebGL2 paths because those are the visually verified live-video implementations. Neural-Pro now tries WebGPU RAVU-Lite below 2x when WebGPU is available, then falls back to WebGL2; near-2x Auto and explicit RAVU-Zoom lazy-load the large LGPL Zoom hook into a separate WebGL2 extension chunk. Neural-Lite can use the shader-native ArtCNN C4F16 WGSL pass chain, the packaged ONNX Runtime path, or the WebGL2 preview path. The current WebGPU shader paths upload frames with `copyExternalImageToTexture`, reuse GPU resources, validate WGSL through Tint to MSL in CI, and use `8x8x1` compute workgroups where compute is active.
