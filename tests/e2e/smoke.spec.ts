...
test('Crisp mode presents dark rendered frames instead of leaving native video on top', async ({
  browserName,
}, testInfo) => {
  test.skip(browserName !== 'chromium', 'Chrome extensions can only be loaded in Chromium.');
  ...
  try {
    context = await createExtensionContext(testInfo.workerIndex + 25);
    await writeExtensionSettings(context, {
      ...DEFAULT_SETTINGS,
      forceWebGL2: true,
      mode: 'crisp',
...
test('Neural-Lite preview changes the rendered WebGL2 output on a paused frame', async ({
  browserName,
}, testInfo) => {
  test.skip(browserName !== 'chromium', 'Chrome extensions can only be loaded in Chromium.');
  ...
  try {
    context = await createExtensionContext(testInfo.workerIndex + 155);
    await writeExtensionSettings(context, {
      ...DEFAULT_SETTINGS,
      forceWebGL2: true,
      hudEnabled: true,
...
test('Neural-Pro RAVU-Lite changes the rendered WebGL2 output on a paused frame', async ({
  ...
  try {
    context = await createExtensionContext(testInfo.workerIndex + 165);
    await writeExtensionSettings(context, {
      ...DEFAULT_SETTINGS,
      forceWebGL2: true,
      hudEnabled: true,
